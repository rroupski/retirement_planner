defmodule RetirementPlanner.Planning do
  ...
  """
  def compound_growth(principal, monthly_contribution, annual_rate, years)
      when is_number(principal) and
  ...
  """
  def required_monthly_savings(current_balance, target_amount, annual_rate, years)
      when is_number(current_balance) and
  ...
  """
  def inflation_adjusted_income(desired_income, inflation_rate, years_until_retirement)
      when is_number(desired_income) and
  ...
  """
  def retirement_nest_egg_needed(annual_income_needed, withdrawal_rate \\ 0.04)
      when is_number(annual_income_needed) and
  ...
  def calculate_portfolio_return(investments) when is_list(investments) do
    total_allocation =
      Enum.reduce(investments, Decimal.new(0), fn inv, acc ->
    ...
    if Decimal.gt?(total_allocation, Decimal.new(0)) do
      ...
      # Default 7% return if no investments defined
      0.07
    end
  end
    ...
    with {:ok, goal} <- get_user_retirement_goal(user_id),
      ...
      current_annual_contributions =
        Enum.reduce(accounts, Decimal.new(0), fn acc, sum ->
          contribution =
            Decimal.add(
              acc.annual_contribution || Decimal.new(0),
  ...
  def delete_retirement_goal(%RetirementGoal{} = goal) do
    Repo.delete(goal)
  end
