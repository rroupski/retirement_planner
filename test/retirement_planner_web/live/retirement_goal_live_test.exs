defmodule RetirementPlannerWeb.RetirementGoalLiveTest do
  ...
  defp create_retirement_goal(_) do
    retirement_goal = retirement_goal_fixture()
    %{retirement_goal: retirement_goal}
  end

  describe "Index" do
    setup [:create_retirement_goal]

    test "lists all retirement_goals", %{conn: conn} do
      {:ok, _index_live, html} = live(conn, ~p"/retirement_goals")

      assert html =~ "Listing Retirement goals"
    end

    test "saves new retirement_goal", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/retirement_goals")

      assert index_live |> element("a", "New Retirement goal") |> render_click() =~
               "New Retirement goal"

      assert_patch(index_live, ~p"/retirement_goals/new")
      ...
      assert_patch(index_live, ~p"/retirement_goals")
    ...
    test "updates retirement_goal in listing", %{conn: conn, retirement_goal: retirement_goal} do
      ...
      assert_patch(index_live, ~p"/retirement_goals/#{retirement_goal}/edit")
  ...
  describe "Show" do
    ...
    test "displays retirement_goal", %{conn: conn, retirement_goal: retirement_goal} do
      {:ok, _show_live, html} = live(conn, ~p"/retirement_goals/#{retirement_goal}")

      assert html =~ "Show Retirement goal"
    end

    test "updates retirement_goal within modal", %{conn: conn, retirement_goal: retirement_goal} do
      {:ok, show_live, _html} = live(conn, ~p"/retirement_goals/#{retirement_goal}")

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Retirement goal"

      assert_patch(show_live, ~p"/retirement_goals/#{retirement_goal}/show/edit")
      ...
      assert_patch(show_live, ~p"/retirement_goals/#{retirement_goal}")
